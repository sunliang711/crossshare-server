...
install(){
    ...
    local rootDir="${dest}/${rootName}"
    if [ ! -d "${rootDir}" ];then
        echo "Create dir: ${rootDir} ..."
        mkdir -p "${rootDir}" || { echo "failed!"; exit 1; }
    fi

    echo "${serviceName} will installed at ${rootDir}"
    ...
    if [ ! -e ${exeName} ];then
        echo "Not found ${exeName},build it first" >&2
        exit 1
    fi

    cp config.toml "${rootDir}"
    mv ${exeName} "${rootDir}"

    ${ed} ${rootDir}/config.toml

    sed -e "s|<DEST>|${rootDir}|g" \
        -e "s|<EXE>|${exeName}|g" \
...
_build(){
    ...
    flags="-X main.buildstamp=`date +%FT%T` -X main.githash=`git rev-parse HEAD` -w -s"
    echo "build ${serviceName}..."
    go build -ldflags "$flags" -o ${exeName} "${this}/../cmd/cross_share_server"
}
